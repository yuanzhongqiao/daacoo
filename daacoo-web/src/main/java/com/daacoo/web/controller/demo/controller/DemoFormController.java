...
import com.daacoo.common.json.JSONObject;
import com.daacoo.common.json.JSONObject.JSONArray;
...
public class DemoFormController {
    ...
    public String localRefreshTask(String fragment, String taskName, ModelMap mmap) {
        JSONArray list = new JSONArray();
        JSONObject item = new JSONObject();
        item.put("name", StringUtils.defaultIfBlank(taskName, "了解各技术的使用、系统使用情况及相关重要追踪人"));
        item.put("type", "新增");
        item.put("date", "2020.08.26");
        list.add(item);
        item = new JSONObject();
        item.put("name", "提高自己技术技巧，灵活、专业地打造高质量的web应用系统");
        item.put("type", "新增");
        item.put("date", "2020.08.09");
        list.add(item);
        mmap.put("tasks", list);
    ...
    public AjaxResult collection() {
        String[] array = {"daacoo 1", "daacoo 2", "daacoo 3", "daacoo 4", "daacoo 5"};
        AjaxResult ajax = new AjaxResult();
